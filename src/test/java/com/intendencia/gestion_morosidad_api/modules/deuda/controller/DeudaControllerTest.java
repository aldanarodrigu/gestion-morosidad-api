package com.intendencia.gestion_morosidad_api.modules.deuda.controller;
...
import com.intendencia.gestion_morosidad_api.modules.deuda.service.DeudaService;
...
class DeudaControllerTest {
    ...
    void listarPasaLosFiltrosAlService() throws Exception {
        ...
        verify(deudaService).listar(eq(new DeudaFiltro(EstadoDeuda.PENDIENTE, "4567", "perez", "Libertad", 2L)), any());
    }
    ...
    @Test
    void listarTributosDeUnaDeuda() throws Exception {
        when(deudaService.listarTributos(1L)).thenReturn(List.of(
                new TributoResponse(1L, "101", null), new TributoResponse(2L, "3802", null)));

        mockMvc.perform(get("/api/deudas/1/tributos"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[1].codigo").value("3802"));
    }

    @Test
    void listarTributosDeDeudaInexistenteDevuelve404() throws Exception {
        when(deudaService.listarTributos(99L)).thenThrow(new RecursoNoEncontradoException("No existe la deuda con id 99"));

        mockMvc.perform(get("/api/deudas/99/tributos"))
                .andExpect(status().isNotFound());
    }
